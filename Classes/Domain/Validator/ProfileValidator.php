...
/**
 * A Validator for posts of the user profile on SugaMines startpage.
 *
 * @scope singleton
 * @package TYPO3
...
class Tx_SugarMine_Domain_Validator_ProfileValidator extends Tx_Extbase_Validation_Validator_AbstractValidator {
	...
	public function isValid($post) {
		...
		foreach ($post['tx_sugarmine_sugarmine'] as $name => $value) {
			...
			if($name !== '__hmac' && $name !== '__referrer' && $name !== 'recordId' && $value !== null && $value !== '') {
				...
				switch($field[1]) {
					...
					default: $error = 'ProfileValidator: The field type "'.$field[1].'" is not existent on the database-table "contacts" of SugarCRM';
				}
		...
		$validFields['passwChange'] = ($passwChange === true) ? true : null;
